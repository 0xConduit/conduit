...
export default function LandingOverlay() {
    ...
    const { login, authenticated, loginError, clearLoginError, isWalletOnlyUser } = useAuth();
    ...
    useEffect(() => {
        ...
        if (isWalletOnlyUser) { router.push('/dashboard'); return; }
        fetch('/api/auth/me')
            .then((res) => (res.ok ? res.json() : null))
            .then((data) => { router.push(data?.user?.onboarded ? '/dashboard' : '/onboarding'); })
            .catch(() => router.push('/dashboard'));
    }, [authenticated, isWalletOnlyUser, router]);
